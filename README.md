...
**Nothing is built yet.** This repository holds the decisions made before any code:

- [`docs/DECISIONS.md`](docs/DECISIONS.md): every approved decision, with the options considered, the reasoning and
  what it costs.
- [`docs/OPEN-QUESTIONS.md`](docs/OPEN-QUESTIONS.md): what is still unresolved, including what the spike must prove.

## Build order

1. **Spike, before any interface.** Prove the risky parts: find Node through the shell's `PATH` from an app opened in
   Finder, run the embedded `prumo new --json` and read its document, run `pnpm mobile` in a pseudo terminal and see
   Expo's QR code, and stop an app leaving no process on its port.
