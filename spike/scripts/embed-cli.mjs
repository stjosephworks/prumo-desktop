...
execFileSync('tar', ['-xzf', tarball, '-C', target, '--strip-components=1'], { cwd: join(root, 'resources') })
rmSync(join(root, 'resources', tarball))
execFileSync('npm', ['install', '--omit=dev', '--ignore-scripts', '--no-package-lock', '--silent'], {
  cwd: target,
  stdio: 'inherit',
})
